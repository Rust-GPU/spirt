...
impl TypeDef {
    ...
    // HACK(eddyb) `pub fn` so that Rust-GPU SPIR-T passes can also rely on this.
    pub fn disaggregated_leaf_count(&self) -> usize {
    ...
    fn try_advance(&mut self) -> bool {
        ...
        for depth in (0..self.parent_component_path.len()).rev() {
            ...
            if let Some(sibling_idx) = ancestor_child_idx.checked_add(1)
                && let Some(sibling_type) = get_component(*ancestor_type, sibling_idx)
            {
...
pub struct DisaggregatedLeafTypes<'a>(Option<AggregateCursor<'a>>);
...
impl Iterator for DisaggregatedLeafTypes<'_> {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        ...
        if let Self(Some(cursor)) = self
            && cursor.parent_component_path.is_empty()
        {
...
impl Type {
    // HACK(eddyb) `pub fn` so that Rust-GPU SPIR-T passes can also rely on this.
    pub fn disaggregated_leaf_types(self, cx: &Context) -> DisaggregatedLeafTypes<'_> {
