...
use crate::{ConstKind, Context, NodeKind, Type, TypeKind, scalar};
...
impl spv::Inst {
    ...
    pub(super) fn from_canonical_type(type_kind: &TypeKind) -> Option<Self> {
        ...
        match type_kind {
            ...
            | TypeKind::Thunk
            | TypeKind::SpvInst { .. }
            | TypeKind::SpvStringLiteralForExtInst => None,
        }
    }
    ...
    pub(super) fn from_canonical_node_kind(node_kind: &NodeKind) -> Option<Self> {
        match node_kind {
            &NodeKind::Scalar(op) => Some(match op {
